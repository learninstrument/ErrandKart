...
-- 1. Add payment_status to orders (errands)
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS payment_status varchar NOT NULL DEFAULT 'unpaid' 
    CHECK (payment_status IN ('unpaid', 'escrow_held', 'released', 'refunded'));

-- 2. Add payment_reference to orders (errands)
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS payment_reference varchar;
