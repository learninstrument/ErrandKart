...
import { ArrowLeft, CheckCircle2, AlertCircle, Truck, Wallet, Bell } from 'lucide-react';
...
const NOTIFICATIONS = [
  ...
  },
  {
    ...
    description: 'KYC document is under review. Usually takes 24 hours.',
      ...
      <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-4 px-6 pb-28 pt-6 lg:grid lg:items-start lg:grid-cols-[1.15fr_0.85fr] lg:gap-8 lg:px-10 lg:pb-10 animate-fade-in-up">
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl ${
                  notification.type === 'warning' ? 'bg-red-500/10 text-red-500' : 'bg-market-green/10 text-market-green'
                }`}>
                  {notification.icon}
                </div>
        ...
        <aside className="flex flex-col gap-6">
          <section className="rounded-3xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#0A0A0A]/80 p-6 shadow-[0_10px_40px_rgba(0,0,0,0.05)] dark:shadow-2xl backdrop-blur-md">
            ...
            <p className="text-sm text-black/60 dark:text-white/60 mb-4">Stay on top of job offers, payouts, and verification updates.</p>
            <div className="flex flex-col gap-3">
              <Button theme="green" className="w-full gap-2 h-11" onClick={() => navigate('/runner/settings')}>
          ...
          <section className="rounded-3xl border border-black/5 dark:border-white/5 bg-white dark:bg-[#0A0A0A]/80 p-6 shadow-[0_10px_40px_rgba(0,0,0,0.05)] dark:shadow-2xl backdrop-blur-md">
            ...
            <div className="flex flex-col gap-3">
              <Button variant="outline" className="w-full gap-2 h-11 border-black/10 dark:border-white/10" onClick={() => navigate('/runner/active')}>
                <Truck size={16} className="text-market-green" /> Open active errand
              </Button>
              <Button variant="outline" className="w-full gap-2 h-11 border-black/10 dark:border-white/10" onClick={() => navigate('/runner/wallet')}>
